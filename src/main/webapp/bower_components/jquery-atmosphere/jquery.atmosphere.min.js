...
h+2&&d.atmosphere.log("error",["Protocol data not sent by the application. If you enable protocol on client side, be sure to install JavascriptProtocol interceptor on application side.Also note that atmosphere-runtime 2.2+ should be used."]);U=parseInt(d.trim(g[h+1]),10);ha=g[h+2];b=!1;"long-polling"!==c.transport&&S(c);d.atmosphere.uuid=c.uuid;f="";h=c.trackMessageLength?4:3;if(g.length>h+1)for(;h<g.length;h++)f+=g[h],h+1!==g.length&&(f+=c.messageDelimiter);0!==c.ackInterval&&setTimeout(function(){Q("...ACK...")},
